...
local function makeDrawingSet()
    ...
    local outline = Drawing.new("Square"); outline.Filled = false; outline.Thickness = 3; outline.Visible = false
    ...
    return { txt = txt, box = box, outline = outline, hpFill = hpFill, hpBack = hpBack, tracer = tracer, dot = dot, hl = hl, skel = skel }
end
...
do
    ...
    local brightness = 2
    local ambR, ambG, ambB = 150, 150, 150
    local ambCol = Color3.fromRGB(ambR, ambG, ambB)
    local writeConn = nil

    local function tick()
        if fullBright then Lighting.Brightness = brightness; Lighting.ClockTime = 14; Lighting.GlobalShadows = false; Lighting.OutdoorAmbient = Color3.fromRGB(150, 150, 150) end
        if noFog then Lighting.FogStart = 1e9; Lighting.FogEnd = 1e9
            local atmos = Lighting:FindFirstChildOfClass("Atmosphere")
            if atmos then atmos.Density = 0; atmos.Haze = 0; atmos.Glare = 0 end
        end
        if ambientOn then Lighting.Ambient = ambCol; Lighting.OutdoorAmbient = ambCol end
    end

    local function refresh()
        if fullBright or noFog or ambientOn then
            if not writeConn then writeConn = RunService.RenderStepped:Connect(tick) end
        else
            if writeConn then writeConn:Disconnect(); writeConn = nil end
            Lighting.Brightness = original.Brightness; Lighting.GlobalShadows = original.GlobalShadows
            Lighting.Ambient = original.Ambient; Lighting.OutdoorAmbient = original.OutdoorAmbient
            Lighting.FogEnd = original.FogEnd; Lighting.FogStart = original.FogStart
        end
    end
    ...
    BloomBox:AddSlider("BloomIntensity", { Text = "Bloom Intensity", Default = 1, Min = 0, Max = 10, Decimals = 0,
        Callback = function(v) if not bloom then bloom = Lighting:FindFirstChildOfClass("BloomEffect") end; if bloom then bloom.Intensity = v end end })

    BloomBox:AddSlider("BloomSize", { Text = "Bloom Size", Default = 24, Min = 0, Max = 56, Decimals = 0,
        Callback = function(v) if not bloom then bloom = Lighting:FindFirstChildOfClass("BloomEffect") end; if bloom then bloom.Size = v end end })
end
...
Library:Notify({
    Title = "Zero Hub",
    Description = "Loaded — Operation One v1.1.0",
    Type = "Success",
    Duration = 3,
})
